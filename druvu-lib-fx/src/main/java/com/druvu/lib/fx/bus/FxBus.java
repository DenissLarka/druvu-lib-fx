...
/**
 * Typed publish/subscribe event bus with explicit threading semantics per subscription (see {@link Delivery}). Events
 * are plain objects - records by convention.
 *
 * <p>Threading contract: {@link #publish(Object)} and {@link #subscribe} may be called from any thread. Dispatch is
 * polymorphic: a subscriber to a supertype receives all subtype events. A handler that throws is logged and never
...
public final class FxBus {
    ...
    private final ThreadFactory asyncThreads =
            Thread.ofVirtual().name("druvu-fx-bus-", 0).factory();

    /**
     * Subscribes a handler for events assignable to the given type. The returned {@link Subscription} must be closed
     * when the subscriber's lifecycle ends.
     */
    ...
    /**
     * Publishes the event to every matching subscriber, from any thread. Returns after CALLER-mode handlers have run;
     * FX/ASYNC deliveries are in flight.
     */
