 ...
 * Changes: repackaged org.dockfx -> com.druvu.lib.fx.dock; inlined the JDK-internal
 * InputEventUtils.recomputeCoordinates to drop the com.sun.* dependency; marked the
 * non-serializable pickResult/contents fields transient. See NOTICE.md.
 **/
 ...
 * events of the same {@code DockEvent} class. Like a {@code MouseEvent} the event will be
...
public class DockEvent extends Event {
   ...
   */
  private transient PickResult pickResult;
   ...
   * pickOnBounds is false.
   */
  private transient Node contents;
  ...
  public DockEvent(Object source, EventTarget target, EventType<? extends DockEvent> eventType,
    ...
    final Point3D p = recomputeCoordinates(this.pickResult);
    this.x = p.getX();
    this.y = p.getY();
    this.z = p.getZ();
  ...
  /**
   * Projects a pick result's intersected point into scene coordinates. This is the behaviour of the
   * JDK-internal {@code com.sun.javafx.scene.input.InputEventUtils.recomputeCoordinates(pick, null)}
   * that upstream DockFX relied on; inlined here so the vendored dock has no {@code com.sun.*}
   * dependency (keeps the module jlink-clean on Java 25).
   *
   * @param pick the pick result to project; must not be null
   * @return the intersected point in scene coordinates, or {@code (NaN, NaN, NaN)} if the pick has
   *         no intersected point
   */
  private static Point3D recomputeCoordinates(PickResult pick) {
    final Point3D point = pick.getIntersectedPoint();
    if (point == null) {
      return new Point3D(Double.NaN, Double.NaN, Double.NaN);
    }
    final Node node = pick.getIntersectedNode();
    return node != null ? node.localToScene(point) : point;
  }
