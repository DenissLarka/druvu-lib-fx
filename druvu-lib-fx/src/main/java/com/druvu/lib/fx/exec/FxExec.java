...
/**
 * Background execution on virtual threads, with task lifecycle published as {@link TaskEvent}s on the bus - so status
 * UIs observe work without the executor knowing about them.
 *
...
public final class FxExec implements AutoCloseable {
    ...
    private final ExecutorService executor = Executors.newThreadPerTaskExecutor(
            Thread.ofVirtual().name("druvu-fx-exec-", 0).factory());
    ...
    /**
     * Interrupts pending tasks and rejects new ones. Worker threads are virtual (daemon), so an unresponsive task never
     * blocks JVM exit either way.
     */
    @Override
    public void close() {
