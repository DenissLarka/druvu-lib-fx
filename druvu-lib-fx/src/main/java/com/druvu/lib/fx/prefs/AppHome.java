...
/**
 * Resolves a per-application directory for user data (preferences, logs, bookmarks, ...). By default this is
 * {@code ~/.druvu.com/<appName>}, so every druvu app shares the {@code ~/.druvu.com} parent - the same convention
 * JConsole Booster established. An optional environment variable relocates it (e.g. to a synced folder), matching JCB's
 * {@code JCONSOLE_BOOSTER_HOME}.
 *
...
public final class AppHome {
     ...
     * @param envVar name of an environment variable that, if set and non-blank, overrides the whole path (like JCB's
     *     {@code JCONSOLE_BOOSTER_HOME}); {@code null} to disable override
