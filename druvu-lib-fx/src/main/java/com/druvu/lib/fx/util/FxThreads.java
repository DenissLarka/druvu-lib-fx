...
/**
 * Thread-affinity primitives for the JavaFX application thread.
 * Every druvu-lib-fx API states its threading contract in these terms.
 */
public final class FxThreads {
	...
	/**
	 * Runs the action on the JavaFX application thread: immediately when the caller is already
	 * there, otherwise via {@link Platform#runLater(Runnable)}.
	 */
	...
	public static void requireFx() {
		if (!isFx()) {
			throw new IllegalStateException("Must be called on the JavaFX application thread, was: " + Thread.currentThread().getName());
		}
	}
	...
	/**
	 * An {@link Executor} dispatching every command via {@link Platform#runLater(Runnable)}.
	 * Composes with {@code CompletableFuture}'s {@code *Async(..., executor)} methods to hop
	 * a continuation onto the FX thread:
