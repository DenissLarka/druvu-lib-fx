...
/**
 * A self-contained sign-in control. It collects a username and password, runs the app-supplied
 * {@link Authenticator} on a background virtual thread (via {@link FxExec}), and on success hands
 * the principal to the app-supplied callback. It deliberately does <em>not</em> decide what happens
 * next - swapping scenes, opening a workspace, etc. is the app's job, driven by {@code onSuccess}.
 *
 * <p>UX states are handled for you: while authenticating, the fields and button disable and the
 * button reads "Signing in..."; a rejected login shows the {@code Authenticator}'s exception
 * message; a fresh attempt clears it.
 *
...
public final class LoginPane<P> extends VBox {
	...
	public LoginPane(FxExec exec, Authenticator<P> authenticator, Consumer<? super P> onSuccess) {
		...
		final VBox form = new VBox(8,
				new Label("Sign in"), usernameField, passwordField, loginButton, errorLabel);
	...
	/**
	 * Attempts a login programmatically. No-op if an attempt is already in flight. Must be called on
	 * the FX thread.
	 *
	...
	public void submit(String username, char[] password) {
		...
		exec.supply("login", () -> authenticator.authenticate(username, password))
				.whenCompleteAsync((principal, failure) -> {
					busy.set(false);
					if (failure == null) {
