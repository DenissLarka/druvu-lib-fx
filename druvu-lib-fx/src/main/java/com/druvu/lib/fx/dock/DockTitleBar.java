 ...
 * Changes: repackaged org.dockfx -> com.druvu.lib.fx.dock; the title-bar drag now honours
 * DockNode.isFloatable() so setFloatable(false) truly pins a node (upstream ignored it, letting a
 * non-closable node be dragged out with no way back). See NOTICE.md.
 **/
...
public class DockTitleBar extends HBox implements EventHandler<MouseEvent> {
  ...
  public void handle(MouseEvent event) {
    if (event.getEventType() == MouseEvent.MOUSE_PRESSED) {
      ...
      if (!dockNode.isFloating()) {
        // A pinned (non-floatable) node must not pop out on a title-bar drag: upstream ignored
        // isFloatable() here, so setFloatable(false) had no effect against dragging and a
        // non-closable pinned node could be detached with no way to re-dock it. Swallow the drag.
