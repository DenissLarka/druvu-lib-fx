...
/**
 * FXML page controller - shows how the toolkit composes with FXML without owning it:
 * the FXML/controller pair is plain JavaFX; FxExec + FxThreads only enter through
 * {@link #init(FxExec)}, called by the shell after loading.
 *
 * Threading: everything here runs on the FX thread except the load callable, which FxExec
 * runs on a virtual thread; results hop back via FxThreads.fxExecutor().
 */
public final class InstrumentsController {
	...
	private void initialize() {
		...
		favColumn.setCellValueFactory(c -> new ReadOnlyStringWrapper(c.getValue().favourite() ? "*" : ""));
		symbolColumn.setCellValueFactory(c -> new ReadOnlyStringWrapper(c.getValue().symbol()));
		nameColumn.setCellValueFactory(c -> new ReadOnlyStringWrapper(c.getValue().name()));
		classColumn.setCellValueFactory(c -> new ReadOnlyStringWrapper(c.getValue().assetClass().name()));
		priceColumn.setCellValueFactory(c -> new ReadOnlyObjectWrapper<>(c.getValue().price()));
		listedColumn.setCellValueFactory(c -> new ReadOnlyStringWrapper(c.getValue().listed().toString()));
