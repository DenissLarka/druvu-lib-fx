...
/**
 * "Market Watch" - the druvu-lib-fx showcase: a fake-data market console laid out as a docking
 * workspace.
 *
 * The app OWNS Application, Stage, Scene and layout - the toolkit only supplies bus, exec, thread
 * primitives, the vendored dock, the {@link LoginPane} and the {@link StatusBarModel}. It opens on
 * a login screen (demo / demo); a successful sign-in swaps the scene root to the docking workspace.
 * Pages demonstrate both styles: code-first (Dashboard, Tasks) and FXML (Instruments). The top
 * toolbar carries one icon toggle per dock panel: pressing it shows the panel (re-docks it),
 * releasing it hides the panel (undocks it); the toggle also follows the panel's own close button.
 */
public final class MarketWatchApp extends Application {
	...
	/**
	 * The login gate. The demo {@link Authenticator} accepts {@code demo / demo} after a short
	 * pretend-network delay (it runs off the FX thread, so blocking here is fine); anything else is
	 * rejected with a message the pane shows. On success the toolkit calls {@link #showWorkspace}.
	 */
	...
	/**
	 * Docks the panels. The show position of each panel is deliberately a side (never CENTER):
	 * {@link DockPane#dock} only honours CENTER as the very first dock, so a panel that is toggled
	 * off and back on re-docks to a side to avoid being silently dropped.
	 */
	private void buildDockLayout() {
		// Each panel gets a two-tone SVG glyph (14px viewBox) in the toolbar.
		panels.add(new Panel("Dashboard", new DashboardPage(bus).node(), DockPos.LEFT,
	...
	/**
	 * A toolbar toggle that shows/hides one dock panel. Selected == panel visible. The listener on
	 * the panel's {@code dockedProperty} keeps the toggle honest when the panel is closed by its own
	 * title-bar button or dragged out to float (both undock it).
	 */
	...
	/**
	 * The status strip binds straight to a kit {@link StatusBarModel} - the task-counting and
	 * last-activity logic that used to live here is now the model's job; the view just binds.
	 */
	private HBox buildStatusStrip(String user) {
		...
		final HBox strip = new HBox(16,
				userLabel, new Separator(Orientation.VERTICAL), tasksLabel, lastLabel);
	...
	private Parent loadInstrumentsPage() {
		try {
			final FXMLLoader loader = new FXMLLoader(
					InstrumentsController.class.getResource("instruments.fxml"));
