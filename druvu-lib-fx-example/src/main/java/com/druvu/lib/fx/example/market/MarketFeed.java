...
/**
 * Simulated market data feed: a random walk over a few symbols, published as {@link Tick}s on the bus from a background
 * virtual thread. Runs as a plain FxExec task, so the toolbar's LIVE indicator and the status strip observe it through
 * ordinary TaskEvents - the feed knows nothing about the UI.
 */
    ...
    private void loop() {
        ...
        try {
            while (running) {
                ...
                final double price = prices.compute(symbol, (s, old) -> old * (1 + random.nextGaussian() * 0.0006));
