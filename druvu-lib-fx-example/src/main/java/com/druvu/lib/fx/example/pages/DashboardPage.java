...
/**
 * Live charts fed by bus {@link Tick}s: % change since feed start per symbol (line) and
 * cumulative traded lots per symbol (bar). The FX-mode subscription is the whole threading
 * story - ticks originate on the feed's virtual thread, handlers land on the FX thread,
 * so chart mutation below needs no locks and no runLater.
 */
public final class DashboardPage {
	...
	public DashboardPage(FxBus bus) {
		...
		final javafx.scene.control.SplitPane split =
				new javafx.scene.control.SplitPane(priceChart, volumeChart);
	...
	private void onTick(Tick tick) {
		...
		final XYChart.Series<Number, Number> series =
				priceSeries.computeIfAbsent(tick.symbol(), this::newPriceSeries);
		series.getData().add(new XYChart.Data<>(tickIndex++, pctChange));
		if (series.getData().size() > WINDOW) {
			series.getData().remove(0);
		}

		final XYChart.Data<String, Number> bar =
				volumeBars.computeIfAbsent(tick.symbol(), this::newVolumeBar);
