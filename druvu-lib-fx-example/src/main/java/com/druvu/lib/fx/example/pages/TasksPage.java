...
/**
 * Background-work playground: launch tasks, watch their {@link TaskEvent} lifecycle stream.
 * Buttons submit from the FX thread and return instantly; the log is filled by an FX-mode
 * bus subscription, so event handling is single-threaded by construction.
 */
public final class TasksPage {
	...
	private static String describe(TaskEvent event) {
		return switch (event) {
			...
			case TaskEvent.Finished f -> "finished  #%d %s in %d ms".formatted(f.id(), f.title(), f.elapsed().toMillis());
			case TaskEvent.Failed x -> "FAILED    #%d %s after %d ms: %s"
					.formatted(x.id(), x.title(), x.elapsed().toMillis(), x.error().getMessage());
		};
	}
}
